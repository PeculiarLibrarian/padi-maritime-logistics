...
export class ClusterManager {
  ...
  constructor(
    ...
    if (protocol !== "rediss:" && process.env.NODE_ENV === "production") {
      throw new Error(
        `REDIS_TLS_REQUIRED: use rediss:// in production (got ${protocol})`
      );
    }
  ...
  async start(): Promise<void> {
    const redisEpoch = parseInt(
      (await this.redis.get(this.epochKey)) ?? "0",
      10
    );
    if (this.engine.currentEpoch > redisEpoch) {
      await this.redis.set(this.epochKey, this.engine.currentEpoch);
      this.engine.log("INFO", "EPOCH_SEEDED_FROM_SNAPSHOT", {
        epoch: this.engine.currentEpoch,
      });
    }
    await this.poll();
  ...
  async poll(): Promise<void> {
    ...
    try {
      ...
      if (acquired === "OK") {
        ...
        this.engine.log("INFO", "LEADER_ELECTED", {
          epoch: this.engine.currentEpoch,
        });
      } else {
      ...
      this.engine.isLeader = false;
      this.engine.log("WARN", "REDIS_POLL_FAIL", {
