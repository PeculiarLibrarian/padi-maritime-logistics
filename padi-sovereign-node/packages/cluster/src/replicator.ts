...
export class Replicator {
  ...
  async sync(): Promise<void> {
    if (this.syncing || this.engine.isLeader || !this.engine.tips.length) {
      return;
    }
    this.syncing = true;
    const peer =
      this.peers[Math.floor(Math.random() * this.peers.length)];
    try {
  ...
  async backfill(peer: string, tipHash: string): Promise<void> {
    ...
    while (currentNeed && !(await this.engine.blockIndex.has(currentNeed))) {
      if (stack.length >= BACKFILL_LIMIT) {
        throw new Error(
          `BACKFILL_LIMIT_EXCEEDED: chain depth > ${BACKFILL_LIMIT}`
        );
      }

      const res   = await this.fetchWithTimeout(
        `${peer}/ledger/block/${currentNeed}`
      );
      ...
      if (blockEpoch < this.engine.currentEpoch - 1) {
        throw new Error(
          `STALE_EPOCH_BLOCK: block.e=${blockEpoch} floor=${this.engine.currentEpoch}`
        );
      }
    ...
    while (stack.length) {
      const b = stack.pop()!;
      if (b.p[0] && !(await this.engine.blockIndex.has(b.p[0]))) {
